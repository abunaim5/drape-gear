...
const DashboardNav = () => {
    ...
    return (
        <div className='flex flex-col w-full md:w-[162px] lg:w-[226px] xl:w-[382px] h-fit border text-sm text-gray-500'>
            {
                dashboardLinks.map((link, idx) => <DashboardLink key={idx} label={link.label} href={link.href} icon={link.icon} userCount={users.length} cartCount={cartItems.length} wishlistCount={itemIds.length} />)
            }
